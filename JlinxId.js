...
class JlinxId {
  constructor (jlinxId) {
    if (typeof jlinxId === 'string') {
      if (jlinxId.startsWith(URL_PREFIX)) { jlinxId = jlinxId.slice(URL_PREFIX.length) }
    ...
    if (jlinxId.byteLength !== sodium.crypto_sign_PUBLICKEYBYTES) {
      throw INVALID_JLINX_ID(
        `expected byteLength to be ${sodium.crypto_sign_PUBLICKEYBYTES} ` +
        `but got ${jlinxId.byteLength}`
      )
    }
